...
extension StringX on String {
  ...
  ///Format String to Phone Number when typing
  ///Example: 01234567890 -> 0123 456 890
  String toPhoneNumber() {
    if (length < 4) return this;
    if (length < 7) return "${substring(0, 4)} ${substring(4)}";
    return "${substring(0, 4)} ${substring(4, 7)} ${substring(7)}";
  }

  String toPhoneNumberStringFromFormat() {
    return replaceAll(' ', '');
  }

  ///Split String (removeDiacritics) by keyword but keep sensitive case of text
  ///Example: 'Hello World'.splitByKeyword('ld') -> '["Hello Wor","ld"]'
  ///Example: 'Nám Quoc Son Ha, nam De Cu'.splitByKeyword('na') -> '["","Ná","m"," Quoc Son Ha, ","na","m"," De Cu"]'
  List<String> splitByKeyword(String keyword) {
    final text = this;
    final keywordText = keyword.removeDiacritics();
    final result = <String>[];
    final textLength = text.length;
    final keywordLength = keywordText.length;
    var start = 0;
    var end = 0;
    while (end < textLength) {
      if (text
              .substring(end, min(end + keywordLength, textLength))
              .toLowerCase()
              .removeDiacritics() ==
        ...
        result.add(text.substring(end, end + keywordLength));
        start = end + keywordLength;
        end = start;
      } else {
        end++;
      }
...
extension StringExt on String? {
  bool get isNotNullOrEmpty {
    ...
    if (this!.isEmpty || this!.trim().isEmpty) return false;
    return true;
  }
}
